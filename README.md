...
This package is written in **TypeScript** and ships its own type declarations, so
there is no need to install a separate `@types/cors` package.
...
The exported types are `CorsOptions`, `CorsOptionsDelegate`, `CorsRequest`,
`CorsResponse`, `CustomOrigin`, `OriginCallback` and `OriginValue`.
...
The library source lives in `lib/index.ts`. The published JavaScript entry point
(`lib/index.js`) and its declaration file (`lib/index.d.ts`) are generated by the
TypeScript compiler.

```sh
$ npm install      # install dependencies
$ npm run typecheck # type-check lib and test sources without emitting
$ npm run build     # compile lib/index.ts -> lib/index.js + lib/index.d.ts
$ npm test          # typecheck, build, then run the test suite with coverage
```

The test suite is written in TypeScript and runs through `mocha` with
`ts-node/register`.
